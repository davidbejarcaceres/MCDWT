...
parser.add_argument("-p" , help="Path to the MCDWT images: /tmp/video_transformed/16bits/", default = "/tmp/video_transformed/16bit/", type = str)
...
if (args.kmeans):
    ...
    for image in range(nImages):
        ...
        for band in bands:
            ...
            subprocess.run("python -O substract_offset.py -i {}{}{:03d}.png -o /tmp/normalized_kmeans.png".format(path, band , image), shell=True, check=True)
            print("Offset Removed from the img to normalized")

            img = cv2.imread("/tmp/normalized_kmeans.png", -1)
            Z = img.reshape((-1,3))
            ...
            cv2.imwrite("/tmp/quantizedKmeans.png", res2.astype(np.uint16)) # Saves the image to tmp indicating the number of steps
            subprocess.run("python3 -O add_offset.py -i /tmp/quantizedKmeans.png -o {}{}{:03d}.png".format(path, band , image), shell=True, check=True)
    ...
    for image in range(nImages):
        ...
        for band in bands:
            print("Band {}".format(band))
            subprocess.run("python -O substract_offset.py -i {}{}{:03d}.png -o /tmp/normalized.png".format(path, band , image), shell=True, check=True)
            print("Offset Removed from the img to normalized")
            ...
            im2Test.save("/tmp/quantized.png","PNG")

            # Reverts to 16 bits images to work with MCDWT project REPLACING THE ORIGINAL IMAGE
            subprocess.run("python3 -O add_offset.py -i /tmp/quantized.png -o {}{}{:03d}.png".format(path, band , image), shell=True, check=True)
            print("Offset Added, sub-band ready!")
